...
<div class="mt-4 flex items-center space-x-4">
    ...
    <form class="like-form my-0" method="POST" action="toggleLike">
        @csrf
        <button type="submit" name="like_button" aria-label="like button" class="@if ($liked)liked text-pink-600 @endif like-btn flex-none focus:outline-0 focus:outline-none accent-link hover:text-pink-600">
    ...
    <button name="reply_button" aria-label="reply button" class="reply-btn flex-none focus:outline-0 focus:outline-none accent-link hover:text-pink-600">
    ...
    @if ($ownsPost or Role::hasPermission($flag, "EDIT_ALL"))
        ...
        <form class="my-0 edit-form" method="PUT" action="posts/edit">
            ...
            <button name="edit_button" aria-label="edit button"  type="button" class="edit-btn flex-none focus:outline-0 focus:outline-none accent-link hover:text-pink-600">
                <svg width=20 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
    ...
    @if ($ownsPost or Role::hasPermission($flag, "DELETE_ALL"))
        <form class="my-0 delete-form" method="POST" action="posts/delete">
            @csrf
            <input class="reply-id" type="hidden" name="post_id" value="{{$post->id}}">
            <button name="delete_button" aria-label="delete button" class="delete-btn flex-none focus:outline-0 focus:outline-none accent-link hover:text-pink-600" type="submit">
                <svg width=20 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
